...
function init(){
	...
	ctx_menu[0].clearRect(0, 0, WIDTH, HEIGHT);
	ctx_menu[1].clearRect(0, 0, WIDTH, HEIGHT);
	
	document.getElementById("info").innerHTML = info;
	
	if( loaded >= 100 ){
		document.getElementById("loading").style.display = "none";
		document.getElementsByClassName("links")[0].innerHTML = "HOME";
		document.getElementsByClassName("links")[1].innerHTML = "SUPPORT";
		document.getElementsByClassName("links")[2].innerHTML = "E-MAIL";
		document.getElementsByClassName("links")[3].innerHTML = "FAQ";
		document.getElementsByClassName("links")[4].innerHTML = "ABOUT";
		
		if( firstLoad === false ){
			firstLoad = true;
			mainBtn.push(new drawBitmap(WIDTH-33, 30*0, 32, 30, ctx_menu, 1, image[2][0], new Border(4, 4, 4, 4, 1)));
			mainBtn.push(new drawBitmap(WIDTH-33, 30*1, 32, 30, ctx_menu, 0, image[5][0], new Border(4, 4, 4, 4, 1)));
			mainBtn.push(new drawBitmap(WIDTH-33, 30*2, 32, 30, ctx_menu, 0, image[5][0], new Border(4, 4, 4, 4, 1)));
			mainBtn.push(new drawBitmap(WIDTH-33, 30*3, 32, 30, ctx_menu, 0, image[5][0], new Border(4, 4, 4, 4, 1)));
			mainBtn.push(new drawBitmap(WIDTH-33, 30*4, 32, 30, ctx_menu, 0, image[7][0], new Border(4, 4, 4, 4, 1)));
		}
...
function drawLayout(){
	...
	//Main Layout
		...
		new drawBitmap(0, 0, WIDTH-28, HEIGHT, ctx_menu, 0, image[1][0], new Border(4, 4, 4, 4, 1)).draw();
	
	ctx_menu[0].fillStyle = 'gray';
	ctx_menu[0].fillRect(6, 6, WIDTH-42, 130);
	...
	this.draw = function(){
		if( Array.isArray(this.ctx) ){
			if( this.border != null && this.border != 'undefined' ){
				...
				this.ctx[this.layer].drawImage(this.img, 0, 0, this.width, this.height);
			}
		} else {
